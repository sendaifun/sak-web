...
export function FeatureCard({ title, description, color }: FeatureCardProps) {
  const borderColor =
    color === "cyan" ? "border-cyan-400" : "border-yellow-400";
  ...
  return (
    <div
      className={`p-6 border ${borderColor} bg-black/50 rounded-lg backdrop-blur-sm 
        hover:bg-black/70 transition-colors group cursor-pointer relative`}
    >
      {" "}
      <div className="absolute inset-0 -z-10">
        <div className="min-h-full bg-[url('/assets/bgs/card.svg')] bg-repeat bg-top"></div>
      </div>
